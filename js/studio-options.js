...
(function (global) {
  ...
  function applyKnob(seq, pattern, menuKey, knobIndex, delta, page, shift) {
    ...
    if (menuKey === 'tempo') {
      ...
      if (knobIndex === 6) { const d = accumulate(seq, 'signature', delta, 4); if (!d) return null; SEQ().applySignature(seq, cycleIndex(SIG_ORDER, seq.signature || '4/4', d)); return 'signature ' + seq.signature; } // #83 time signature (auto-fits start/end)
      return null;
    }

    if (menuKey === 'pattern') {
      // start/end are limited to the signature's step count (#83): e.g. 3/4 → 1-12.
      const max = stepsPerBar(seq, pattern) - 1;
